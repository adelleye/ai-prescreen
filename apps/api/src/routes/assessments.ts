...
import { SubmitAnswerRequest, NextQuestionRequest } from '@shared/core';
import type { GradeOutcome } from '../services/scoring';
import { getQuestionText } from '@shared/core';
import {
  checkStopRules,
 ...
 */
function mapContextFields<T extends Record<string, unknown>>(
  source: Record<string, unknown>,
  mapping: Record<string, string>,
): Partial<T> {
  return Object.entries(mapping).reduce((acc, [sourceKey, targetKey]) => {
    if (source[sourceKey] !== undefined) {
      (acc as Record<string, unknown>)[targetKey] = source[sourceKey];
    }
    return acc;
  }, {} as Partial<T>);
}
...
const registerAssessments: FastifyPluginAsync<{
  ...
  app.post('/submit', {
    config: {
      rateLimit: {
        ...
        keyGenerator: (req: any) => {
          ...
          const aid = session?.assessmentId || (req.body as any)?.assessmentId || 'unknown';
          return `assess:${aid}`;
        },
      },
    },
    ...
    try {
      const result = await withTransaction(async (client) => {
        // Lock assessment row
        const { rows: arows } = await client.query<{ started_at: string | null; finished_at: string | null; job_id: string }>(
  ...
  app.post('/next-question', {
    ...
    if (!parsed.success) {
      ...
      sendError(reply, 400, 'BadRequest', parsed.error.errors.map(e => `${e.path.join('.')}: ${e.message}`).join('; '), req);
      return;
    }
